...
def initializeFeatureMatrices(bResetFiles=False, bPostProcessing=True, bNormalize=True):
    ...
    try:
        ...
        print("this is the size of the fControl:")
        print(np.shape(fControl))
        print(fControl)
        ...
        print("Splitting features, this is the size of labelfile")
        print(np.shape(labelfile))
        # print(labelfile)
...
def postProcessFeatures(mFeatures, mControlFeatures):
    ...
    print("Are there any NaNs after postProcessing?")
    print(np.any(np.isnan(mFeatures)))
    # DEBUG LINES
    print("Data shape after replacement: %s" % (str(np.shape(mFeatures))))
    #############
    print("Replacing NaNs from feature set... Done.")

    print("This is mFeatures in postProcessing...")
    print(mFeatures)
    return mFeatures
...
def splitFeatures(clinicalfile, datafile, labelfile):
    ...
    print("This is the label file:")
    print(labelfile)
    print("This is the shape of the labelfile: %s", np.shape(labelfile))
    ...
    #############
    print("Splitfeatures: This is the mFeatures...")
    print(mFeatures)
...
def loadTumorStage():
    ...
    print("This is the clininal file...")
    print(clinicalfile)
    print("These are the dimensions of the clininal file")
    print(np.shape(clinicalfile))
    return clinicalfile
...
def getControlFeatureMatrix(mAllData, vLabels):
    print("Finding only the control data...")
    choicelist = mAllData
    condlist = isEqualToString(vLabels, 'Solid_Tissue_Normal')
    print("This is the control feature matrix:")
    print(choicelist[condlist])
    print("Data shape: %s" % (str(np.shape(choicelist))))
    print("Finding only the control data...Done")
...
# Calculating relative change and applying a log 2 norm
...
def getFeatureGraph(mAllData, dEdgeThreshold=0.30, bResetGraph=True, dMinDivergenceToKeep=np.log2(10e5)):
    ...
    for iFirstFeatIdx, iSecondFeatIdx in lCombinations:
        qCombination.put(
            (iFirstFeatIdx, iSecondFeatIdx, g, mAllData, saFeatures, iFirstFeatIdx, iSecondFeatIdx, dEdgeThreshold))
...
def getGraphAndData(bResetGraph=False, dMinDivergenceToKeep=np.log2(10e6), dEdgeThreshold=0.3):
    ...
    return mFeatures_noNaNs, vClass , saRemainingFeatureNames, gToDraw
...
def getAvgShortestPath(gGraph):
    try:
        ...
        mShortestPaths = np.asarray(
            [nx.average_shortest_path_length(g) for g in nx.connected_component_subgraphs(gGraph)])
        ...
        for nCurNode in gGraph.nodes():
            ...
            for nNeighborNode in gGraph[nCurNode]:
                ...
                try:
                    ...
                    if bAbsoluteMass:
                        gGraph.node[nNeighborNode]['weight'] = abs(gGraph.node[nNeighborNode]['weight']) + abs(
                            dMassToMove)
                    else:
...
def generateAllSampleGraphFeatureVectors(gMainGraph, mAllSamples, saRemainingFeatureNames):
    ...
    return np.apply_along_axis(
        lambda mSample: getSampleGraphFeatureVector(gMainGraph, mSample, saRemainingFeatureNames), 1, mAllSamples)
...
def main():
    # # main function
    gMainGraph, mFeatures_noNaNs, vClass, saRemainingFeatureNames = getGraphAndData(bResetGraph=False,
                                                                                    dEdgeThreshold=0.3,
                                                                                    dMinDivergenceToKeep=6)
    # vGraphFeatures = getGraphVector(gMainGraph)
