...
def initializeFeatureMatrices(bResetFiles=False, bPostProcessing=True, bNormalize=True,
    ...
    try:
        ...
        labelfile = np.genfromtxt(fControl, skip_header=1, usecols=(0, 73682),
...
def splitFeatures(clinicalfile, datafile, labelfile):
    ...
    mFeaturesOnly = datafile[:, 1:73682]
...
def loadPatientAndControlData():
    ...
    datafile = np.genfromtxt(fControl, skip_header=1, usecols=range(1, 73682),
...
def loadTumorStage():
    ...
    fClinical = open("./clinical.tsv", "r")
    # While loading stage, also convert string to integer
    clinicalfile = np.genfromtxt(fClinical, skip_header=1, usecols=(1, 127),
                                 missing_values=['NA', "na", '-', '--', 'n/a'], delimiter="\t",
                                 dtype=np.dtype("object"),
...
def getFeatureNames():
    ...
    fControl = open("./featureVectors_singleTest.txt", "r")
    # fControl = open("/datastore/cvas/output.txt", "r")
...
def main(argv):
    ...
    parser.add_argument("-fvf", "--featureVectorsFilename", default="featureVectors_229.txt")
    ...
    # Update global feature vector filename
    FEATURE_VECTOR_FILENAME = args.featureVectorsFilename
